/*
  ...
  gcc  -g -o mandelbrot -O4 mandelbrot.c
  Usage:
...
int main(int argc, char *argv[])
{
  ...
  // int* arr = (int*)malloc((atoi(argv[5])) * sizeof(unsigned char [6]));
  ...
  // unsigned char arr[yres][6];
  // unsigned char *arr = (char *)malloc(yres * sizeof(char[6]));

  //  char **arr = (char **)calloc(yres, sizeof(char *));

  char **arr;

  arr = (char **)malloc(yres * sizeof(char *));
  for (int i = 0; i < yres; i++)
  {
    arr[i] = (char *)malloc(6 * sizeof(char));
  }

  // for (int zz = 0; zz < yres; zz++)
  // {
  //   arr[zz] = (char *)calloc(6, sizeof(char));
  // }
  ...
  for (j = 0; j < yres; j++)
  {
    ...
    for (i = 0; i < xres; i++)
    {
      ...
      if (k >= maxiter)
      {
        ...
        {
          arr[arr_index][iterator] = black[iterator];
        }
      ...
      else
      {
        ...
        for (int iterator = 0; iterator < 6; iterator++)
        {
          arr[arr_index][iterator] = color[iterator];
        }
  ...
  for (int iterator = 0; iterator < yres; iterator++)
  {
    fwrite(arr[iterator], 6, 1, fp);
  }
